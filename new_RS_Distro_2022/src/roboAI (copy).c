...
//  T[104][PENALTY_SHOOT_COMPLETE] = 104;
 ...
 **************************************************************************/
  ...
  *****************************************************************************/
  ...
  ai->DPhead = addCross(ai->DPhead, enemy_goal_x, sy/2, 30, 0, 0, 255);
...
void penalty_get_to_target(struct RoboAI *ai)
{
  ...
  else
  {
    ...
    ai->DPhead = addVector(ai->DPhead, self_x, self_y, vec2tgt_x, vec2tgt_y, 200, 0, 255, 0);
    ai->DPhead = addVector(ai->DPhead, self_x, self_y, enemy_goal_x-self_x, (sy/2)-self_y, 200, 255, 255, 0);
    ...
    // penalty_drive_left = std::min(100, std::max(-100, penalty_drive_left));
    // penalty_drive_right = std::min(100, std::max(-100, penalty_drive_right));
    // fprintf(stderr, "left_motor_speed: %d right_motor_speed: %d\n", penalty_drive_left, penalty_drive_right);
    int drive_left = penalty_drive_left - u;
    int drive_right = penalty_drive_right + u;
